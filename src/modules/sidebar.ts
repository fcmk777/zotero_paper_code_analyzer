...
import { normalizeGitHubRepoUrl } from "../context/repo/github-url";
...
const ZOTERO_TOOL_MANUAL = [
  ...
  "- For paper-code analysis, prefer the repo_* V2 tools: import GitHub archive into a local Repo Workspace, inspect repo_get_module_map and repo_get_symbol_skeleton, then read specific files with repo_read_file_with_lines or repo_read_related_files. Do not rely on old raw GitHub reads when a workspace exists.",
  "- Code evidence must cite only real paths and line numbers returned by repo_read_file_with_lines or repo_read_related_files. If evidence is missing, say it is not found instead of guessing.",
].join("\n");
...
interface WindowSidebarState {
  ...
  itemSelectionMonitorID?: number;
  lastRenderedItemID?: number | null;
...
interface PanelState {
  ...
  codeRepoLoaded?: boolean;
  codeRepoDraft: string;
  codeRepoRefDraft: string;
  codeRepoStatus?: string;
  codeRepoStatusError?: boolean;
  codeAnalysisSections: string[];
...
function renderMount(mount: HTMLElement, itemID: number | null) {
  ...
  if (!state || state.itemID !== itemID) {
    ...
    state = {
      ...
      codeRepoLoaded: false,
      codeRepoDraft: "",
      codeRepoRefDraft: "",
      codeAnalysisSections: CODE_ANALYSIS_SECTIONS.slice(),
    };
...
function renderPanel(mount: HTMLElement, state: PanelState) {
  ...
  try {
    ...
    panel.append(renderCodeRepositoryPanel(doc, mount, state));
...
function renderCodeRepositoryPanel(
  doc: Document,
  mount: HTMLElement,
  state: PanelState,
): HTMLElement {
  loadBoundRepoWorkspaceOnce(mount, state);

  const card = el(doc, "div", "code-repo-card");
  card.append(
    el(doc, "div", "code-repo-title", "论文代码仓库"),
    el(
      doc,
      "div",
      "code-repo-help",
      "粘贴 GitHub URL 后导入索引；分支/tag/commit 直接用 /tree/<ref> URL。",
    ),
  );

  const input = inputEl(doc, state.codeRepoDraft || "");
  input.className = "code-repo-input";
  input.placeholder = "https://github.com/owner/repo 或 .../tree/main";
  input.disabled = state.itemID == null;
  input.addEventListener("input", () => {
    state.codeRepoDraft = input.value;
    state.codeRepoStatus = undefined;
    state.codeRepoStatusError = false;
  });

  const row = el(doc, "div", "code-repo-row");
  row.append(input);
  card.append(row);

  const actions = el(doc, "div", "code-repo-actions code-repo-actions-compact");
  const status = buttonEl(doc, "状态");
  status.disabled = state.itemID == null || state.sending;
  status.addEventListener("click", () => {
    void sendMessage(
      mount,
      state,
      "请调用 repo_get_workspace_status 查看当前论文已导入的 Repo Workspace 状态，并用中文简要说明文件规模、语言分布、主要 warning，以及下一步是否可以生成代码讲解。",
      { taskTitle: "查看工程索引状态" },
    );
  });
  ...
  const analyze = buttonEl(doc, "生成并写入笔记");
  analyze.disabled = state.itemID == null || state.sending;
  analyze.addEventListener("click", () => {
    void generateCodeAnalysisToNoteFromSidebar(mount, state);
  });

  const view = buttonEl(doc, "查看已保存");
  view.disabled = state.itemID == null || state.sending;
  view.addEventListener("click", () => {
    void showSavedCodeAnalysisInChat(mount, state);
  });

  actions.append(analyze, status, rebuild, view);
  card.append(actions);

  const sections = el(doc, "div", "code-sections");
  for (const section of CODE_ANALYSIS_SECTIONS) {
    const label = el(doc, "label", "code-section");
    const checkbox = doc.createElement("input");
    checkbox.type = "checkbox";
    checkbox.checked = state.codeAnalysisSections.includes(section);
    checkbox.addEventListener("change", () => {
      state.codeAnalysisSections = checkbox.checked
        ? Array.from(new Set([...state.codeAnalysisSections, section]))
        : state.codeAnalysisSections.filter((value) => value !== section);
    });
    label.append(checkbox, doc.createTextNode(section));
    sections.append(label);
  }
  const details = doc.createElement("details");
  details.className = "code-sections-details";
  const summary = doc.createElement("summary");
  summary.className = "code-sections-summary";
  summary.textContent = `分析范围（已选 ${selectedCodeAnalysisSections(state).length} 项）`;
  details.append(summary, sections);
  card.append(details);

  if (state.codeRepoStatus) {
    card.append(
      el(
        doc,
...
function loadBoundRepoWorkspaceOnce(mount: HTMLElement, state: PanelState): void {
  ...
  void loadBoundRepoWorkspace(state.itemID).then((result) => {
    ...
    if (result.ok && !state.codeRepoDraft) {
      state.codeRepoDraft = `https://github.com/${result.workspace.owner}/${result.workspace.repo}`;
      state.codeRepoRefDraft = result.workspace.ref;
      state.codeRepoStatus = `已导入索引：${result.workspace.owner}/${result.workspace.repo}@${result.workspace.ref}，${result.workspace.indexedTextFiles}/${result.workspace.totalFiles} 个文本文件。`;
      state.codeRepoStatusError = false;
      renderPanel(mount, state);
    }
  });
...
async function importRepoWorkspaceFromSidebar(
  mount: HTMLElement,
  state: PanelState,
  forceReimport: boolean,
): Promise<void> {
  if (state.itemID == null) return;
  const parsed = normalizeGitHubRepoUrl(state.codeRepoDraft);
  if (!parsed.ok) {
    state.codeRepoStatus = parsed.message;
    state.codeRepoStatusError = true;
    renderPanel(mount, state);
    return;
  }
  state.codeRepoDraft = parsed.normalizedUrl;
  state.codeRepoStatus = forceReimport
    ? "正在通过工具流重建 Repo Workspace 索引..."
    : "正在通过工具流导入 GitHub archive 并建立本地索引...";
  state.codeRepoStatusError = false;
  renderPanel(mount, state);
  await sendMessage(
    mount,
    ...
    { taskTitle: forceReimport ? "重建仓库索引" : "导入仓库索引" },
  );
}

async function generateCodeAnalysisToNoteFromSidebar(
  mount: HTMLElement,
  state: PanelState,
): Promise<void> {
  if (state.itemID == null || state.sending) return;
  const itemID = state.itemID;
  const initialWorkspace = await loadBoundRepoWorkspace(itemID);
  if (states.get(mount) !== state) return;
  const draftUrl = state.codeRepoDraft.trim();
  let githubUrl = "";
  let visibleRepo = "当前已导入 Repo Workspace";
  if (initialWorkspace.ok) {
    githubUrl = `https://github.com/${initialWorkspace.workspace.owner}/${initialWorkspace.workspace.repo}`;
    visibleRepo = `${initialWorkspace.workspace.owner}/${initialWorkspace.workspace.repo}@${initialWorkspace.workspace.ref}`;
  } else {
    const parsed = normalizeGitHubRepoUrl(draftUrl);
    if (!parsed.ok) {
      state.codeRepoStatus = `${initialWorkspace.message} 请先在 GitHub URL 输入框粘贴公开仓库地址，然后点击“生成并写入笔记”。`;
      state.codeRepoStatusError = true;
      renderPanel(mount, state);
      return;
    }
    ...
    visibleRepo = `${parsed.owner}/${parsed.repo}${parsed.ref ? `@${parsed.ref}` : ""}`;
  }

  const sections = selectedCodeAnalysisSections(state);
  const fullPrompt = buildCodeAnalysisPrompt(githubUrl, sections);
  const visiblePrompt = [
    `生成代码讲解并写入笔记：${visibleRepo}`,
    `分析范围：${sections.join(" / ")}`,
  ].join("\n");
  state.codeRepoStatus = initialWorkspace.ok
    ? "正在生成完整代码讲解；聊天区只显示进度，完成后自动写入 Zotero 子笔记。"
    : "正在一键导入 GitHub archive、建立索引并生成完整代码讲解；完成后自动写入 Zotero 子笔记。";
  state.codeRepoStatusError = false;
  renderPanel(mount, state);

  await sendMessage(mount, state, visiblePrompt, {
    taskTitle: "生成并写入笔记",
    apiContent: fullPrompt,
    suppressAssistantText: true,
    onFinalAssistantContent: async (markdown) => {
      const finalWorkspace = await loadBoundRepoWorkspace(itemID);
      if (!finalWorkspace.ok) {
        throw new Error(
          `仓库导入或索引保存失败，未写入笔记。${finalWorkspace.message}`,
        );
      }
      ...
      const result = await appendMarkdownSourceToItemNote(
        mount.ownerDocument!,
        itemID,
        noteMarkdown,
      );
      return result.created
        ? `已生成完整代码讲解，保存到本地 report storage，并新建 Zotero 子笔记 #${result.noteID}。`
        : `已生成完整代码讲解，保存到本地 report storage，并写入 Zotero 子笔记 #${result.noteID}。`;
    },
  });
}

async function saveLatestCodeAnalysisFromSidebar(
  mount: HTMLElement,
  state: PanelState,
): Promise<void> {
  if (state.itemID == null) return;
  const markdown = lastAssistantMarkdown(state);
  if (!markdown) {
    state.codeRepoStatus = "没有可保存的 assistant 分析内容。";
    state.codeRepoStatusError = true;
    renderPanel(mount, state);
    return;
  }
  const workspace = await loadBoundRepoWorkspace(state.itemID);
  if (!workspace.ok) {
    state.codeRepoStatus = workspace.message;
    state.codeRepoStatusError = true;
    renderPanel(mount, state);
    return;
  }
  ...
  if (!saved.ok) {
    ...
    state.codeRepoStatus = `已保存代码讲解报告（${saved.report.markdown.length} 字）。`;
    state.codeRepoStatusError = false;
  }
  renderPanel(mount, state);
}

async function writeLatestCodeAnalysisToNoteFromSidebar(
  mount: HTMLElement,
  state: PanelState,
): Promise<void> {
  if (state.itemID == null) return;
  const markdown = lastAssistantMarkdown(state);
  if (!markdown) {
    state.codeRepoStatus = "没有可写入笔记的 assistant 分析内容。";
    state.codeRepoStatusError = true;
    renderPanel(mount, state);
    return;
  }
  ...
  try {
    const result = await appendAssistantContentToItemNote(
      mount.ownerDocument!,
      state.itemID,
      formatCodeAnalysisForObsidianNote(
        markdown,
        `https://github.com/${workspace.workspace.owner}/${workspace.workspace.repo}`,
      ),
    );
    refreshVisibleNoteWindow(mount.ownerDocument!, result.noteID);
    state.codeRepoStatus = result.created
      ? `已新建 Zotero 笔记并写入代码分析（Note #${result.noteID}）。`
      : `已追加代码分析到 Zotero 笔记 #${result.noteID}。`;
    state.codeRepoStatusError = false;
  } catch (err) {
    state.codeRepoStatus = `写入笔记失败：${errorMessage(err)}`;
    state.codeRepoStatusError = true;
  }
  renderPanel(mount, state);
}

async function showSavedCodeAnalysisInChat(
  mount: HTMLElement,
  state: PanelState,
): Promise<void> {
  const workspace = await loadBoundRepoWorkspace(state.itemID);
  ...
  if (!loaded.ok) {
    state.codeRepoStatus = loaded.message;
    state.codeRepoStatusError = true;
    renderPanel(mount, state);
    return;
  }
  const updated = new Date(loaded.report.updatedAt).toLocaleString();
  state.codeRepoStatus = [
    `已保存讲解：${loaded.report.owner}/${loaded.report.repo}@${loaded.report.ref}`,
    `${loaded.report.markdown.length} 字，更新时间 ${updated}。`,
    "完整内容已在 report storage 中；如需重新写入 Zotero 笔记，请重新点击“生成并写入笔记”。",
  ].join(" ");
  state.codeRepoStatusError = false;
  renderPanel(mount, state);
}

function formatCodeAnalysisForObsidianNote(
  markdown: string,
  githubUrl: string,
): string {
  ...
  return [
    ...
    "",
    "> [!tip] 阅读说明",
    "> 代码证据以 `文件路径:Lx-Ly` 或代码块首行 `# path Line x-y` 为准。若报告标注“不确定”，表示当前已读取代码不足以支持强结论。",
    "",
    `生成时间：${now}`,
    "",
    "---",
...
function selectedCodeAnalysisSections(state: PanelState): string[] {
  return state.codeAnalysisSections.length
    ? state.codeAnalysisSections
    : CODE_ANALYSIS_SECTIONS.slice();
}

function lastAssistantMarkdown(state: PanelState): string {
...
function buildRepoImportPrompt(
  githubUrl: string,
  ref: string,
  forceReimport: boolean,
): string {
  return [
    "请导入当前论文对应的 GitHub 仓库并建立本地工程索引。",
    "",
    `GitHub URL：${githubUrl}`,
    ref ? `Ref：${ref}` : "Ref：留空，使用 GitHub 默认分支",
    `forceReimport：${forceReimport ? "true" : "false"}`,
    "",
    "请按顺序调用 repo_import_github_archive，并在完成后调用 repo_get_workspace_status。",
    "请用中文报告：解析 URL、下载 archive、解压扫描、索引文件数、语言分布、跳过文件原因、warnings，以及下一步是否可以生成代码讲解。",
    "如果导入失败，请给出具体错误 code、原因和下一步修复建议，尤其说明是否是代理超时、GitHub 限流、仓库不存在或 ref 不存在。",
  ].join("\n");
}
...
function buildCodeAnalysisPrompt(githubUrl: string, sections: string[]): string {
  ...
  return [
    "请基于当前 Zotero 论文和当前已导入的 GitHub Repo Workspace，生成一份中文 Markdown 代码级解读报告。",
    "",
    repoLine,
    `本次选择的分析维度：${sections.join(" / ")}`,
    "",
    "你必须按以下顺序工作：",
    "",
    "1. 先调用 paper_get_context_digest；如果不存在，则调用 paper_build_context_digest。",
    "2. 调用 repo_get_workspace_status，确认当前 item 已绑定成功导入的 repo workspace；如果还没有 workspace 且上方提供了 GitHub URL，先调用 repo_import_github_archive，再重新调用 repo_get_workspace_status。",
    "3. 调用 repo_get_module_map 和 repo_get_symbol_skeleton，先理解整个工程结构，不要一上来只读单个文件。",
    "4. 根据用户选择的分析维度，调用 repo_search_files、repo_read_related_files、repo_read_file_with_lines 精读关键源码。",
    "5. 必要时调用 paper_search_sections 或 paper_read_section，把论文方法/实验描述与代码实现对齐。",
    "6. 最后只输出完整 Markdown 报告正文。不要调用 analysis_save_report；保存和写入 Zotero 笔记由本地“生成并写入笔记”按钮流程完成。",
    "",
    "输出风格要求：",
    "- 面向 Obsidian/本地笔记阅读，结构清晰，优先使用二级/三级标题、简短段落、表格、Mermaid 和少量关键代码块。",
    "- 不要把工具返回的长源码整段复制到报告里；每个代码块控制在 8-30 行，首行用注释标明 `# path  Line x-y`。",
    "- 对重要提醒使用 Obsidian callout，例如 `> [!warning]`、`> [!tip]`。",
    "- 如果读取文件被截断，并且关键类/方法可能在后半段，必须继续调用 repo_search_files 或 repo_read_file_with_lines 读取真实行段。",
    "- 对 `forward`、`__init__`、训练循环、loss 计算等关键逻辑，不允许只根据 import 或文件名推断；找不到就明确写未找到。",
    "- 最终回答不要包含“我将保存/是否保存”等对话语句，只输出可直接导入 Obsidian 的 Markdown 报告正文。",
    "",
    "# 论文代码仓库解读报告",
    "",
    "## 1. 论文与仓库基本信息",
    "- 论文标题、作者、年份",
    "- GitHub 仓库、ref/commit、导入文件规模",
    "- 本次分析覆盖了哪些代码区域",
    "- 哪些区域未覆盖或被跳过",
    "",
    "## 2. 工程整体结构",
    "- 用目录树/模块表说明仓库如何组织",
    "- 标出 README、配置、模型、训练、数据、loss、评测、推理相关文件",
    "- 说明你判断这些文件角色的依据",
    "",
    "## 3. 模型架构代码解读",
    "- 按输入 → 特征处理/主干网络/核心模块 → 输出的数据流讲解",
    "- 必须引用真实文件路径和真实行号",
    "- 不能只说概念，要指出类、函数、forward/核心调用位置",
    "",
    "## 4. 训练流程代码解读",
    "- 训练入口在哪里",
    "- dataset/dataloader 如何构造",
    "- model 如何创建",
    "- loss/objective 如何计算",
    "- optimizer/scheduler/checkpoint/eval 如何连接",
    "- 必须引用真实文件路径和真实行号",
    "",
    "## 5. 论文创新点与代码实现对应关系",
    "对每个创新点按如下格式写：",
    ...
    "- 实现逻辑：这段代码如何对应论文方法",
    "- 证据强度：强 / 中 / 弱",
    "- 如果没找到：写“未在已读取代码中找到直接实现”",
    "",
    "## 6. 数据流与调用链",
    "- 从数据读取到模型输出画出文字版 pipeline",
    "- 如果可以，给出关键调用链：train.py → dataset → model → loss → eval",
    "- 必须基于真实代码证据",
    "",
    "## 7. 复现与阅读建议",
    "- 最应该优先阅读的 5-10 个文件",
    "- 每个文件为什么重要",
    "- 复现实验应该先看哪些配置和脚本",
    "- 可能的坑：环境、数据、权重、路径、配置、版本",
    "",
    "## 8. 不确定性与未验证部分",
    "- 明确列出没有读取到或无法确认的内容",
    "- 不要编造任何不存在的文件、函数、类、行号",
    "",
    "严格要求：",
    "1. 所有代码证据必须来自 repo_read_file_with_lines 或 repo_read_related_files 返回内容。",
    "2. 不得编造文件名、函数名、类名、行号。",
    "3. 找不到就写“未在已读取代码中找到直接实现”。",
    "4. 不要把整篇论文和整个仓库源码一次性塞进上下文。",
    "5. 先用 module map/symbol skeleton 建立全局理解，再精读关键文件。",
    "6. 结论必须区分“代码证据支持”和“基于结构/文件名的合理推测”。",
    "7. 输出要适合本科生/科研实习面试准备：既讲清工程结构，也能应对老师追问代码逻辑。",
  ].join("\n");
}
...
interface SendMessageOptions {
  ...
  onFinalAssistantContent?: (content: string) => Promise<string> | string;
}
...
async function sendMessage(
  ...
  const apiContent = options.apiContent?.trim();
  const wireContent = apiContent || baseContent;
  const preset = selectedChatPreset(state);
  const images = state.draftImages
    .filter((image) => wireContent.includes(image.marker))
    .map((image) => ({ ...image }));
  ...
  const userMessage: Message = {
    role: "user",
    content: baseContent,
    ...(apiContent && apiContent !== baseContent ? { apiContent } : {}),
  ...
  await streamAssistant(mount, state, history, userMessage, {
    ...
    onFinalAssistantContent: options.onFinalAssistantContent,
  });
  void processNextQueuedChatTask(mount, state);
}
...
async function streamAssistant(
  ...
  const isLiveState = () => states.get(mount) === state;
  const renderPanelIfLive = () => {
    if (isLiveState()) renderPanel(mount, state);
  };
  const updateMessageBubbleIfLive = (index: number, message: Message) => {
    if (isLiveState()) updateMessageBubble(mount, index, message);
  };
  const suppressAssistantText = options.suppressAssistantText === true;
  let generatedAssistantContent = "";
  ...
  try {
    ...
    const toolSettings = loadToolSettings(zoteroPrefs());
    ...
    for await (const chunk of getProvider(preset).stream(
      ...
      if (chunk.type === "text_delta") {
        ...
        generatedAssistantContent += chunk.text;
    ...
    toolSession?.dispose();
    if (
      options.onFinalAssistantContent &&
      generatedAssistantContent.trim() &&
      !controller.signal.aborted &&
      !userMessage.task?.error
    ) {
      try {
        assistant.content = await options.onFinalAssistantContent(
          generatedAssistantContent.trim(),
        );
      } catch (err) {
        const message = errorMessage(err);
        markMessageTaskError(userMessage, message);
        assistant.content = `生成完成，但写入 Zotero 笔记失败：${message}`;
      }
    }
...
function getActiveReader(win: Window | null | undefined): any {
  ...
  if (tabs?.selectedType && tabs.selectedType !== "reader") return null;
...
async function appendMarkdownSourceToItemNote(
  doc: Document,
  itemID: number | null,
  markdown: string,
): Promise<{ noteID: number; created: boolean; usedBetterNotes: boolean }> {
  if (itemID == null) throw new Error("未选择 Zotero 条目");
  const target = await resolveTargetNote(itemID);
  const html = markdownSourceToNoteHTML(doc, markdown);
  const usedBetterNotes = await insertHTMLIntoNote(target.note, html);
  return {
    noteID: target.note.id,
    created: target.created,
...
function markdownSourceToNoteHTML(doc: Document, markdown: string): string {
  const root = doc.createElement("div");
  root.append(doc.createElement("hr"));

  const title = doc.createElement("h2");
  title.textContent = `AI 代码讲解 ${formatNoteTimestamp(new Date())}`;
  root.append(title);

  const hint = doc.createElement("p");
  hint.textContent =
    "以下内容按 Obsidian Markdown 源文本写入，导入 Obsidian 后可渲染标题、表格、callout、Mermaid 和代码块。";
  root.append(hint);

  const pre = doc.createElement("pre");
  pre.className = "zai-obsidian-markdown-source";
  pre.textContent = markdown.trim();
  root.append(pre);
  return String(root.innerHTML);
}
...
export function registerSidebarForWindow(win: Window) {
  ...
  startItemSelectionMonitor(win, state);
...
export function unregisterSidebarForWindow(win: Window) {
  ...
  stopItemSelectionMonitor(win, state);
...
function renderWindowSidebar(win: Window) {
  ...
  state.lastRenderedItemID = itemID;
  updateToggleButton(state);
}
...
function startItemSelectionMonitor(win: Window, state: WindowSidebarState): void {
  ...
  state.itemSelectionMonitorID = win.setInterval(() => {
    if (windowSidebars.get(win) !== state) return;
    const itemID = safeSelectedItemID(win);
    if (itemID === state.lastRenderedItemID) return;
    renderWindowSidebar(win);
  }, 300);
}
