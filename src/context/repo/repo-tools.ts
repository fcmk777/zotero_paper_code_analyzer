  ...
  return [
    {
      ...
      parameters: objectSchema(
        {
          githubUrl: stringSchema("GitHub repository URL, tree URL, blob URL, or git@github.com:owner/repo.git."),
          ref: stringSchema("Optional branch, tag, or commit sha. Overrides URL ref when provided."),
          forceReimport: booleanSchema("If true, rebuild the workspace even when cached."),
        },
        ["githubUrl"],
      ),
      execute: async (args) => safeExecute(async () => {
        ...
        const bound = await bindRepoWorkspaceToItem(itemID, saved.workspace.workspaceId);
        ...
        return jsonResult(
          {
            ...
            itemID,
            cached: false,
            message: "已导入 GitHub archive，完成本地扫描和索引，并绑定到当前论文。",
          },
    ...
    },
    {
      ...
      parameters: objectSchema({}),
      execute: async () => safeExecute(async () => {
        const loaded = await loadBoundWorkspace(options);
        if (!loaded.ok) return errorResult(loaded);
        return jsonResult(
          { ok: true, itemID: loaded.itemID, ...workspaceSummary(loaded.workspace) },
    ...
    },
    {
      ...
      parameters: objectSchema({
        roleTags: arraySchema("Optional role tags such as model_architecture, training_entry, dataset_pipeline, loss_objective, config."),
        language: stringSchema("Optional language filter, e.g. python, markdown, yaml."),
      ...
      execute: async (args) => safeExecute(async () => {
        ...
        const limit = clamp(
          Math.floor(numberArg(parsed, "limit") ?? policy.maxManifestEntriesReturned),
          1,
          policy.maxManifestEntriesReturned,
        );
        const entries = loaded.workspace.manifest
          .filter((meta) => !roleTags.length || roleTags.some((tag) => meta.roleTags.includes(tag as never)))
    ...
    },
    {
      ...
      execute: async (args) => safeExecute(async () => {
        ...
        const parsed = objectArgs(args);
        const roleTags = stringArrayArg(parsed, "roleTags");
        const limit = clamp(Math.floor(numberArg(parsed, "limit") ?? 40), 1, 100);
    ...
    },
    {
      ...
      parameters: objectSchema(
        {
          query: stringSchema("Keywords, class names, function names, paper method terms, or file path fragments."),
      ...
      execute: async (args) => safeExecute(async () => {
        ...
        if (!query) return errorResult(repoError("INVALID_ARGUMENT", "repo_search_files 需要非空 query。", true));
      ...
      parameters: objectSchema(
        {
          ...
          includeContext: booleanSchema("Reserved for future expansion; currently ignored."),
        },
        ["path"],
      ),
      execute: async (args) => safeExecute(async () => {
        ...
        if (!file) {
          return errorResult(repoError("FILE_READ_FAILED", `未在 workspace 中找到文件：${path}`, true));
        }
    ...
    },
    {
      ...
      parameters: objectSchema(
        {
          role: stringSchema("Role tag, e.g. model_architecture, training_entry, dataset_pipeline, loss_objective, config."),
      ...
      execute: async (args) => safeExecute(async () => {
        ...
        const limit = clamp(
          Math.floor(numberArg(parsed, "limit") ?? policy.maxReadRelatedFiles),
          1,
    ...
    },
    {
      ...
      parameters: objectSchema({
        forceRebuild: booleanSchema("If true, rebuild digest even when cached."),
        includeAnnotations: booleanSchema("Whether to include Zotero annotations summary."),
      }),
      execute: async (args) => safeExecute(async () => {
        const itemID = currentItemID(options);
        if (itemID == null) {
          return errorResult(repoError("NO_CURRENT_ZOTERO_ITEM", "当前没有选中的 Zotero 条目。", true));
        }
        const parsed = objectArgs(args);
        if (!booleanArg(parsed, "forceRebuild")) {
          ...
          if (cached.ok) {
            return jsonResult(
              { ok: true, digest: digestForOutput(cached.digest), cached: true },
    ...
    },
    {
      ...
      execute: async (args) => safeExecute(async () => {
        ...
        const parsed = objectArgs(args);
        const query = stringArg(parsed, "query");
        const limit = clamp(Math.floor(numberArg(parsed, "limit") ?? 5), 1, 12);
    ...
    },
    {
      ...
      parameters: objectSchema(
        {
          sectionTitle: stringSchema("Section title substring, e.g. Method, Experiments, Results."),
      ...
      execute: async (args) => safeExecute(async () => {
        ...
        const budget = clamp(
          Math.floor(numberArg(parsed, "charBudget") ?? policy.maxPaperSectionChars),
          1000,
        ...
        if (!section?.text) {
          return errorResult(repoError("NO_PAPER_SECTION", `没有找到论文章节：${title}`, true));
        }
...
async function buildPaperDigest(
  ...
  if (!metadata) {
    return repoError("NO_CURRENT_ZOTERO_ITEM", "无法读取当前 Zotero 条目元数据。", true);
  }
  ...
  const digest: PaperContextDigest = {
    ...
    methodKeywords: keywordsFromText(
      [
        ...
        metadata.abstract ?? "",
        sections
          .filter((section) => /method|approach|model|architecture|framework/i.test(section.title))
    ...
    experimentKeywords: keywordsFromText(
      sections
        .filter((section) => /experiment|result|ablation|evaluation/i.test(section.title))
        .map((section) => section.summary ?? "")
        .join(" "),
      20,
    ),
    ...(annotations.length
      ? {
          annotationsSummary: annotations
            .slice(0, 80)
            .map((annotation) =>
              [annotation.pageLabel ? `p.${annotation.pageLabel}` : "", annotation.text, annotation.comment]
                .filter(Boolean)
                .join(" | "),
            )
            .join("\n")
...
function sectionDigest(
  ...
  const offsets = matches
    ...
    .filter((entry, index, list) => index === 0 || entry.startOffset - list[index - 1].startOffset > 500);
  if (!offsets.length) {
...
function searchWorkspace(
  ...
  return workspace.manifest
    .map((meta) => {
      ...
      const roleBoost = roleTags.filter((tag) => meta.roleTags.includes(tag as never));
    ...
    .filter((result) => result.matchedTerms.length || result.snippetLines.length || roleTags.some((tag) => result.roleTags.includes(tag as never)))
...
function errorResult(error: RepoToolError): ToolExecutionResult {
  return jsonResult(error, String(error.message || "Repo Workspace 工具执行失败。"));
}
