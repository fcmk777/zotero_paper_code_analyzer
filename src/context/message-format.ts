...
export function formatUserMessageForApi(
  ...
  const apiContent = messageApiContent(message);
  const contextBlocks = formatContextBlocks(
    message,
    fullText,
    options.includeTurnInstructions !== false,
  );
  if (!contextBlocks.length) return apiContent;
  return [...contextBlocks, "[User question]", apiContent].join("\n");
}

function messageApiContent(message: Message): string {
