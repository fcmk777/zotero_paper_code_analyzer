...
function presetRow(doc: Document, preset: ModelPreset): HTMLElement {
  ...
  testBtn.addEventListener("click", async (event) => {
    ...
    try {
      ...
      const saved = {
        ...result.preset,
        extras: { ...result.preset.extras, testStatus: "ok" as const },
      };
      ...
      const failed = {
        ...rawPreset,
        extras: { ...rawPreset.extras, testStatus: "failed" as const },
      };
...
function renderToolSettings(doc: Document): void {
  ...
  const githubToken = byID<HTMLInputElement>(doc, "zai-tool-github-token");
...
function readToolSettingsControls(doc: Document): ToolSettings {
  ...
  return {
    ...
    githubToken:
      byID<HTMLInputElement>(doc, "zai-tool-github-token")?.value.trim() ??
