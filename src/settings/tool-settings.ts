...
export function normalizeToolSettings(value: unknown): ToolSettings {
  ...
  return {
    ...
    githubToken: normalizeGitHubToken(input.githubToken),
...
function normalizeGitHubToken(value: unknown): string {
  ...
  // GitHub PATs are opaque enough that overly strict validation causes
  // false negatives; just strip whitespace and cap absurd pasted blobs.
