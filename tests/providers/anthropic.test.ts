...
describe("AnthropicProvider", () => {
  ...
  it("chat path: returns null when extras has no reasoningEffort (legacy preset)", () => {
    expect(
      buildAnthropicThinking({ ...preset, extras: { vendor: "claude" } }),
    ).toBeNull();
  ...
  it("catalog drives suggestions: MODEL_SUGGESTIONS is derived from MODEL_CATALOG", () => {
    expect(MODEL_SUGGESTIONS.claude).toEqual(
      MODEL_CATALOG.claude.map((m) => m.id),
    );
    expect(MODEL_SUGGESTIONS.openai).toEqual(
      MODEL_CATALOG.openai.map((m) => m.id),
    );
    expect(MODEL_SUGGESTIONS.deepseek).toEqual(
      MODEL_CATALOG.deepseek.map((m) => m.id),
    );
  });
