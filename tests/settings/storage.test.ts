...
describe("preset storage", () => {
  ...
  it("detectAnthropicVendor falls back to compat for unknown urls and models", () => {
    ...
    expect(detectAnthropicVendor("https://example.com/v1", "mystery")).toBe(
      "compat",
    );
    expect(
      detectAnthropicVendor("https://api.deepseek.com/anthropic", "mystery"),
    ).toBe("deepseek");
    expect(
      detectAnthropicVendor("https://api.anthropic.com", "claude-haiku-4-5"),
    ).toBe("claude");
  });
