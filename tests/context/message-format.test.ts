...
describe('toApiMessages', () => {
  it('uses apiContent for the provider while keeping visible content separate', () => {
    const currentMessage: Message = {
      role: 'user',
      content: '正在生成并写入 Zotero 笔记',
      apiContent: '完整代码分析 prompt',
    };

    const messages = toApiMessages([currentMessage], {
      message: currentMessage,
    });

    expect(currentMessage.content).toBe('正在生成并写入 Zotero 笔记');
    expect(messages[0].content).toBe('完整代码分析 prompt');
  });

  it('wraps context around apiContent rather than the visible prompt', () => {
    const currentMessage: Message = {
      role: 'user',
      content: '短进度',
      apiContent: '完整问题',
      context: { selectedText: 'Selected evidence.' },
    };
    ...
    expect(messages[0].content).toContain('Selected evidence.');
    expect(messages[0].content).toContain('[User question]\n完整问题');
    expect(messages[0].content).not.toContain('[User question]\n短进度');
  });
